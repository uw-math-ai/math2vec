...
# Write dict of results {Original -> Translation} to CSV
field_names = ["ORIGINAL", "TRANSLATED"]
time_stamp = time.strftime("%Y-%m-%d_%H-%M-%S")
with open(f"{translations_output_dir}/theorem_translations_{time_stamp}_{len(theorems)}_{model_safe}_v2.csv", mode="w", newline="", encoding="utf-8") as csvfile:
    writer = csv.DictWriter(csvfile, fieldnames=field_names)
    writer.writeheader()
    for original, translated in results.items():
         writer.writerow({"ORIGINAL": original, "TRANSLATED": translated})

# Write a simple log file for later reference
with open(f"{log_output_dir}/run_log_{time_stamp}_{model_safe}_v2.txt", "w") as log:
